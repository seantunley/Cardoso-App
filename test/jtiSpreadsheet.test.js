...
function buildAndParse({ rows, manual }) {
  const buf = buildJtiWorkbook({ rows, manual });
...
describe('deriveDocumentType', () => {
  it('returns Invoice for documents containing "IN"', () => {
    expect(deriveDocumentType('IN000428009')).toBe('Invoice');
    expect(deriveDocumentType('IN1')).toBe('Invoice');
  });

  it('matches case-insensitively (the macro uses Excel SEARCH which is)', () => {
    expect(deriveDocumentType('in000428009')).toBe('Invoice');
    expect(deriveDocumentType('In12345')).toBe('Invoice');
  });

  it('returns Credit Note when "IN" is absent', () => {
  ...
  it('returns Credit Note for null / undefined / non-string inputs (defensive)', () => {
  ...
  it('returns Invoice if "IN" appears anywhere in the string (mirrors macro SEARCH)', () => {
...
describe('buildJtiWorkbook — sheet shape', () => {
  it('produces a single sheet named exactly "Sheet1"', () => {
    const { sheetNames } = buildAndParse({ rows: [SAMPLE_ROW_1], manual: SAMPLE_MANUAL });
    expect(sheetNames).toEqual([JTI_SHEET_NAME]);
    expect(JTI_SHEET_NAME).toBe('Sheet1');
  });

  it('emits exactly 14 columns in the header row, in the documented order', () => {
    const { ws } = buildAndParse({ rows: [SAMPLE_ROW_1], manual: SAMPLE_MANUAL });
  ...
  it('produces a header-only sheet when rows are empty (not a 404)', () => {
    const { ws } = buildAndParse({ rows: [], manual: SAMPLE_MANUAL });
...
describe('buildJtiWorkbook — data row layout', () => {
  it('places each source field in the correct column for row 2', () => {
    const { ws } = buildAndParse({ rows: [SAMPLE_ROW_1], manual: SAMPLE_MANUAL });
  ...
  it('preserves leading whitespace in ProductName (macro/Crystal artefact)', () => {
    const row = { ...SAMPLE_ROW_1, DESC: '   GLAMOUR*SUPERSLIM* PINK' };
    const { ws } = buildAndParse({ rows: [row], manual: SAMPLE_MANUAL });
    expect(ws['E2'].v).toBe('   GLAMOUR*SUPERSLIM* PINK');
  });

  it('keeps the row order from the input array (no internal sorting)', () => {
    ...
    const { ws } = buildAndParse({ rows: out_of_order, manual: SAMPLE_MANUAL });
    expect(ws['D2'].v).toBe('B');
    expect(ws['D3'].v).toBe('A');
    expect(ws['D4'].v).toBe('C');
  });

  it('applies the same TownCity/Region/Country to every row', () => {
    const { ws } = buildAndParse({ rows: [SAMPLE_ROW_1, SAMPLE_ROW_2], manual: SAMPLE_MANUAL });
  ...
  it('coerces null manual fields to empty strings (defensive)', () => {
    const { ws } = buildAndParse({
      rows: [SAMPLE_ROW_1],
  ...
  it('handles null NAMECUST without rendering "null" text (LEFT JOIN may produce null)', () => {
    const orphan = { ...SAMPLE_ROW_1, NAMECUST: null };
    const { ws } = buildAndParse({ rows: [orphan], manual: SAMPLE_MANUAL });
...
describe('buildJtiWorkbook — number formats', () => {
  it('Date column (C) is a number cell with format "0"', () => {
    const { ws } = buildAndParse({ rows: [SAMPLE_ROW_1], manual: SAMPLE_MANUAL });
    expect(ws['C2'].t).toBe('n');
    expect(ws['C2'].z).toBe('0');
  });

  it('Quantity column (K) is a number cell with format "#,##0.00"', () => {
    const { ws } = buildAndParse({ rows: [SAMPLE_ROW_1], manual: SAMPLE_MANUAL });
    expect(ws['K2'].t).toBe('n');
    expect(ws['K2'].z).toBe('#,##0.00');
  });

  it('does NOT format the header row (matches macro behaviour — header stays General)', () => {
    const { ws } = buildAndParse({ rows: [SAMPLE_ROW_1], manual: SAMPLE_MANUAL });
    expect(ws['C1'].z === undefined || ws['C1'].z === 'General').toBe(true);
    expect(ws['K1'].z === undefined || ws['K1'].z === 'General').toBe(true);
  });

  it('Date stays as integer YYYYMMDD, NOT converted to an Excel date value', () => {
    const { ws } = buildAndParse({ rows: [SAMPLE_ROW_1], manual: SAMPLE_MANUAL });
...
describe('buildJtiWorkbook — placeholder columns L/M/N', () => {
  it('CostExVAT / NetValueExVAT / Currency cells are absent (not blank strings)', () => {
  ...
  it('column count of the sheet stays at 14 even though L/M/N are absent', () => {
...
describe('buildJtiWorkbook — input validation', () => {
  it('throws TypeError when rows is not an array', () => {
    expect(() => buildJtiWorkbook({ rows: null, manual: SAMPLE_MANUAL })).toThrow(TypeError);
    expect(() => buildJtiWorkbook({ rows: 'oops', manual: SAMPLE_MANUAL })).toThrow(TypeError);
  });

  it('throws TypeError when manual fields are missing', () => {
    expect(() => buildJtiWorkbook({ rows: [], manual: null })).toThrow(TypeError);
    expect(() => buildJtiWorkbook({ rows: [] })).toThrow(TypeError);
  });
});

describe('buildJtiWorkbook — DocumentType edge cases', () => {
  it('emits Credit Note for a credit-note document', () => {
    const cn = { ...SAMPLE_ROW_1, TRANNUM: 'CN000123456' };
    const { ws } = buildAndParse({ rows: [cn], manual: SAMPLE_MANUAL });
    expect(ws['A2'].v).toBe('Credit Note');
  });

  it('emits Credit Note when DocumentNumber is empty', () => {
