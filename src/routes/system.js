...
export function createSystemRouter({ requireAuth, requireAdmin }) {
  ...
  // POST /api/maintenance/prune-orphan-extractions — admin-only, site-only,
  // dry-run/apply. Heals the `no_orphan_extractions` integrity failure: an
  // extraction row whose order_number is NOT in BAT's Overview pivot for its
  // recon (usually a stale row left from an upload before the automatic prune
  // existed). Such rows inflate the bucket sums and make the week fail to
  // reconcile. Dry-run lists exactly which rows would be removed before any
  // delete; the per-recon banner / integrity report confirms the fix afterwards.
  router.post('/api/maintenance/prune-orphan-extractions', requireAuth, requireAdmin, (req, res) => {
    try {
      if (process.env.HUB_MODE === 'true') {
        return res.status(400).json({ error: 'Orphan-extraction prune is site-only.' });
      }
      const dryRun = req.body?.dryRun !== false;
      const orphans = db.prepare(`
        SELECT e.id AS ext_id, e.order_number, e.is_exception, e.order_amount, e.pdf_url,
        ...
        ORDER BY r.year DESC, r.week_number DESC, e.order_number
      `).all();

      let removed = 0;
      if (!dryRun && orphans.length > 0) {
        const del = db.prepare('DELETE FROM bat_invoice_extractions WHERE id = ?');
        db.transaction(() => { for (const o of orphans) { del.run(o.ext_id); removed += 1; } })();
      }

      const byReconMap = new Map();
      for (const o of orphans) {
        const k = `${o.year}-${o.week_number}`;
        if (!byReconMap.has(k)) byReconMap.set(k, { week_number: o.week_number, year: o.year, count: 0, amount: 0 });
        const g = byReconMap.get(k);
        g.count += 1; g.amount += Number(o.order_amount) || 0;
      }
      const byRecon = [...byReconMap.values()];

      logAudit({
        req,
        action: dryRun ? 'bat_prune_orphan_extractions_dryrun' : 'bat_prune_orphan_extractions',
        resourceType: 'system',
        resourceName: 'BAT orphan-extraction prune',
        details: dryRun
