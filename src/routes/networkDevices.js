...
export function createNetworkDevicesRouter({ requireAuth, requireAdmin, requirePermission }) {
  ...
  router.get('/api/network-devices', requireAuth, requirePermission('can_access_network_devices'), (req, res) => {
    try {
      const sampleHours = Math.min(Math.max(parseInt(req.query.sample_hours, 10) || 48, 6), 168);
      res.json(getNetworkDevicesSnapshot({ sampleHours }));
    } catch (error) {
  ...
  router.post('/api/network-devices/scan', requireAuth, requirePermission('can_access_network_devices'), async (req, res) => {
    try {
      const result = await scanNetworkDevicesNow({ triggerReason: req.currentUser?.email ? `manual:${req.currentUser.email}` : 'manual' });
      res.json(result);
    } catch (error) {
  ...
  router.get('/api/hub/network-devices', requireAuth, requirePermission('can_access_network_devices'), (req, res) => {
    try {
      if (process.env.HUB_MODE !== 'true') {
        return res.json({ devices: [], samples: [], sites: [] });
      }
      const sampleHours = Math.min(Math.max(parseInt(req.query.sample_hours, 10) || 48, 6), 168);
      const siteId = typeof req.query.site_id === 'string' && req.query.site_id.trim() ? req.query.site_id.trim() : null;
      const snapshot = getHubNetworkDevicesSnapshot({ siteId, sampleHours });
  ...
  router.post('/api/hub/network-devices/pull', requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const allSites = db.prepare('SELECT id, slug, name, url, token FROM hub_sites ORDER BY name ASC, slug ASC').all();
      const targetSites = rawSiteIds?.length ? allSites.filter((site) => rawSiteIds.includes(site.id)) : allSites;
      ...
      res.status(results.every((row) => row.ok) ? 200 : 207).json({ results });
    } catch (error) {
