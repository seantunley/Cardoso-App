...
define({
  ...
  label: 'Creditors — UNPOSTED vendor payments (AP batches + cashbook)',
  purpose: 'Vendor payments captured but not yet posted into APOBL, from BOTH stages of the payment pipeline: AP payment batches awaiting posting, and Peresoft Cashbook entries with vendor allocations whose batch has not been transferred to AP yet. Until posting, APOBL still shows the paid invoices as open. The Creditor Balances page nets these off.',
  pool: 'bat_sage',
  tables: ['APTCR', 'APBTA', 'CBBTSD', 'CBBTHD', 'APVEN'],
  params: [],
  ...
  // Both stages verified against live Sage (CARDAT). The pipeline is
  // Cashbook-first: payments are captured in Peresoft Cashbook and the CB
  // transfer GENERATES the AP payment batch (posted batches literally say
  // "Generated by CB") — so a captured payment is invisible to APOBL at
  // either stage. The stages are mutually exclusive (the CB entry flips
  // SWPOSTED=1 when it transfers and creates the AP batch), so the UNION
  // can't double-count one payment.
  //   Stage 1 (AP): APTCR.BTCHTYPE 'PY' + CNTBTCH ↔ APBTA.PAYMTYPE + CNTBTCH;
  //   "unposted" = POSTSEQNBR = 0 AND BATCHSTAT <> 4 (not deleted) —
  //   deliberately NOT a status-code list, so unsampled codes can't break it.
  //   Stage 2 (CB): CBBTSD vendor allocations under unposted cashbook headers
  //   (SWPOSTED = 0). CBBTSD.IDCUST carries CUSTOMERS on receipt entries and
  //   VENDORS on payment entries — the APVEN join keeps only vendors.
  //   batch_status -1 marks the cashbook stage.
  defaultSql: `
  SELECT
  ...
  UNION ALL
  SELECT
    LTRIM(RTRIM(s.IDCUST))      AS vendor_code,
    LTRIM(RTRIM(s.PAYNUMBER))   AS payment_number,
    h.[DATE]                    AS payment_date_int,
    s.APPLAMOUNT                AS amount,
    CAST(h.BATCHID AS int)      AS batch_number,
    -1                          AS batch_status,
    'Cashbook (not yet transferred to AP): ' + LTRIM(RTRIM(h.TEXTDESC)) AS batch_description
  FROM CBBTSD s
  JOIN CBBTHD h ON h.BATCHID = s.BATCHID AND h.ENTRYNO = s.ENTRYNO
  JOIN APVEN v ON LTRIM(RTRIM(v.VENDORID)) = LTRIM(RTRIM(s.IDCUST))
  WHERE h.SWPOSTED = 0 AND s.APPLAMOUNT <> 0
`,
});
